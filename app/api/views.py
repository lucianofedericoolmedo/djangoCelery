...
from worker.tasks import fogar_alta_garantia_prestamo
...
def index(request, format=None):
    if request.method == 'POST':
        ...
        return Response('Ok index', status=status.HTTP_201_CREATED)
    files = [] if not os.path.exists(settings.DATA_PATH) else os.listdir(settings.DATA_PATH)
    return Response(files)

@api_view(['GET', 'POST'])
def alta_garantia(request, format=None):
    if request.method == 'POST':
        fogar_alta_garantia_prestamo.s(
            garantia=request.data['garantia']).delay()
        return Response('Altas encoladas OK', status=status.HTTP_201_CREATED)
    files = [] if not os.path.exists(settings.DATA_PATH) else os.listdir(settings.DATA_PATH)
    return Response(files)
